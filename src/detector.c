...
void validate_detector_recall(char* datacfg, char* cfgfile, char* weightfile)
{
   ...
   /*     int* nDetPtr = &nboxes;
        merge(dets, nDetPtr, detStore, 2);
        nboxes = *nDetPtr;
        dets = convertFromDetObj(detStore->store, detStore->storeLength);*/
...
/*
Uses detection store
*/
float validate_detector_map2(char* datacfg, char* cfgfile, char* weightfile, float thresh_calc_avg_iou, const float iou_thresh, const int map_points, int letter_box, network* existing_net)
{
    int j;
    list* options = read_data_cfg(datacfg);
    char* valid_images = option_find_str(options, "valid", "data/train.txt");
    ...
    printf(valid_images); //results/video/imagefiles.txt
    printf("\n");
    list* plist = get_paths(valid_images);
    char** paths = (char**)list_to_array(plist);
    ...
    for (i = nthreads; i < m + nthreads; i += nthreads) {
        ...
        for (t = 0; t < nthreads && i + t - nthreads < m; ++t) {
            ...
            float* X = val_resized[t].data;
            network_predict(net, X);
            //printf("%s\n", path); //path is path + filename of current image to process
...
void run_detector(int argc, char** argv)
{
    ...
    //name_list: data/wider/wider.names
    ...
    else if (0 == strcmp(argv[2], "map2")) validate_detector_map2(datacfg, cfg, weights, thresh, iou_thresh, map_points, letter_box, NULL);
    ...
    else if (0 == strcmp(argv[2], "anonfiles")) {
        printf("Anonymize files\n");

        anonfiles(datacfg, cfg, weights, thresh, hier_thresh, filename, names, classes, prefix, out_filename,
            dont_show, ext_output, letter_box, outfile, letter_box);
    }
