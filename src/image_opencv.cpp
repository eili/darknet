...
extern "C" {
   ...
   */
    ...
    */
    ...
    void drawEllipse(cv::Mat* mat, cv::Point center, int h, int w)
    {
        cv::Scalar color;
        ...
        try {
            ...
            for (i = 0; i < num; ++i) {
                ...
                for (j = 0; j < classes; ++j) {
                   // printf("j=%d, ", j);
                    int show = strncmp(names[j], "dont_show", 9);
                    if (det.prob[j] > thresh && show) {
                        if (class_id < 0) {
                            strcat(labelstr, names[j]);
                            class_id = j;
                            char buff[10];
                            sprintf(buff, " (%2.0f%%)", det.prob[j] * 100);
                            strcat(labelstr, buff);
                            classProb = det.prob[j];
                        }
                        else {
                            strcat(labelstr, ", ");
                            strcat(labelstr, names[j]);
                        ...
                        //class_id>=0
                        printf("%s: %.0f%% ", names[j], det.prob[j] * 100);
                        classProb = det.prob[j];
                    }
                }
                if (class_id >= 0) {
                  ...
                  /*  if (detObj.missCount == 0) {
                    ...
                    float xfactor = detObj.deltaX * detObj.missCount;
                    float yfactor = detObj.deltaY * detObj.missCount;
                    int left = (xfactor + (b.x - b.w / 2.)) * show_img->cols;
                    int right = (xfactor + (b.x + b.w / 2.)) * show_img->cols;
                    int top = (yfactor + b.y - b.h / 2.) * show_img->rows;
                    int bot = (yfactor + b.y + b.h / 2.) * show_img->rows;
